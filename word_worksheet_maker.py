...
class DownloadImage(DownloadImage):
    ...
    def start_makedWS(self):
            ...
            if self.picture_on:
                ...
                if not os.path.exists(pic):
                    ...
                    q.information(
                        self,
                        "information",
                        "선택하신 이미지가 존재하지 않습니다. 사진을 다시 업데이트합니다..",
                        QMessageBox.Ok,
                    )
                    return
        ...
        if self.path:
            worksheet_worker = WorksheetWorker(
                WordCardWorksheet,
                word_image,
                self.width_spin.value(),
                self.path,
                syllable=self.syllable_checkBox.isChecked(),
            )
    ...
    def finish_makedobblePpt(self):
        ...
        q.information(
            self,
            "information",
            "단어 활동지를 완성했습니다. 파일은 {}에 저장했습니다. 이용해주셔서 감사합니다^ㅇ^".format(self.path),
            QMessageBox.Ok,
        )
    ...
    def get_save_word_wordsheet_dir(self):
        ...
        if is_dir_path:
            ...
            if "WS_dir" in data.keys():
                ...
                q = QMessageBox(self)
                q.information(
                    self, "information", "단어 활동지를 저장할 폴더를 선택하세요.", QMessageBox.Ok
                )

...
class MakeWordSearch:
    ...
    def col(self, word):
        right = 1
        up = 0
        word_set = self.show_me_the_way(
            right, up
        )  # 0:start_x_code, 1:start_y_code, 2:direction_x_code, 3:direction_y_code
    ...
    def make_puzzle(self, filename="puzzle"):
        ...
        if difficulty == 1:
            ...
            self.difficulty = (
                "random.choice([self.col, self.col_rev, self.row, self.row_rev])(word)"
            )
        elif difficulty == 3:
            self.difficulty = (
                "random.choice([self.col, self.row, self.diagup, self.diagdown])(word)"
            )
        elif difficulty == 4:
                ...
                if self.puzzle_origin[i][j] == "0":
                    ...
                    # 글자가 겹치도록 하기 위해서 많이 나온 글자와 무작위 글자들 중에서 고르도록 한다.
                        ...
                        puzzle[i][j] = random.choice(
                            [fill_alph, random.choice(count_alpha.most_common(7))[0]]
                        )
                    printed_words += puzzle[i][j]
        ...
        if os.path.exists("hwp_settings.json"):
            with open("hwp_settings.json") as f:
                data = json.load(f)
                para_belong = document.add_paragraph(
                    "{}학년 {}반 이름: _______".format(data["grade"], data["class"])
                )
        else:
            para_belong = document.add_paragraph("__학년 __반 이름: _______")
        para_belong.alignment = WD_ALIGN_PARAGRAPH.RIGHT

        puzzle_table = document.add_table(
        ...
        if self.pic_on:
            ...
            hint_table = document.add_table(
                rows=(len(words) + size - 1) // size * 2, cols=size, style="Table Grid"
            )
                            ...
                            if i % 2 == 1:
                                ...
                                if self.chosung_scramable:
                                    ...
                                    if self.korean:
                                        cho_word = ""
                                        for chr in word:
                                            chosung_scramable = hgtk.letter.decompose(
                                                chr
                                            )[0]
        ...
        # 정답 파일 쓰기
        answ_doc = Document()
        answer_table = answ_doc.add_table(
            rows=self.height, cols=self.width, style="Table Grid"
        )
